...
class ConstructionDocument(Base):
    ...
    document_type: Mapped[str | None] = mapped_column(String(50), nullable=True)  # e.g. blueprint, contract, inspection
    caption: Mapped[str | None] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(TIMESTAMPTZ(timezone=True), server_default=func.now())

    project: Mapped["ConstructionProject"] = relationship(back_populates="documents")  # type: ignore[name-defined]  # noqa: F821
